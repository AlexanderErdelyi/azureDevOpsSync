...
class CronScheduler {
  ...
  async executeSync(configId) {
    ...
    // Use SyncEngine for consistent execution with logging
    ...
    console.log(`CronScheduler: Execution ${result.execution_id} completed - ${result.results.created + result.results.updated} synced, ${result.results.errors} failed`);
  }
  ...
  async getScheduledJobs() {
    ...
    for (const [configId, task] of this.jobs) {
      ...
      jobs.push({
        configId: Number(configId), // Ensure numeric ID
