...
class SyncEngine {
  ...
  async _executeUnidirectionalSync(
    ...
    const mappings = await this.mappingEngine.loadMappings(this.config.id);
  ...
  async _executeBidirectionalSync(executionId, workItemIds, dryRun) {
    ...
    // Step 1: Sync new items from source to target (items not yet paired)
    this.logger.info('Step 1: Querying new items from source');
    const sourceWorkItems = await this._queryWorkItems(this.sourceConnector, workItemIds);
    
    // Get all synced pairs to filter out items that are already paired (in either direction)
    const allPairs = await db('synced_items')
      .where({ sync_config_id: this.config.id })
      .select('source_connector_id', 'source_item_id', 'target_connector_id', 'target_item_id');
    
    // An item from source is "new" if it doesn't appear in any existing pair
    const newSourceItems = sourceWorkItems.filter(item => {
      return !allPairs.some(pair => 
        (pair.source_connector_id === this.config.source_connector_id && pair.source_item_id === item.id.toString()) ||
        (pair.target_connector_id === this.config.source_connector_id && pair.target_item_id === item.id.toString())
      );
    });
    this.logger.info(`Found ${newSourceItems.length} new items in source to sync to target`);
    ...
    for (const sourceItem of newSourceItems) {
      try {
        ...
        results.total++;
        if (itemResult.created) results.created++;
        if (itemResult.updated) results.updated++;
        if (itemResult.skipped) results.skipped++;
        results.items.push(itemResult);
        
        // Log the sync action
        if (itemResult.action === 'created') {
          const title = sourceItem.fields?.['System.Title'] || sourceItem.title || 'Untitled';
          const sourceUrl = this.sourceConnector.getWorkItemUrl ? this.sourceConnector.getWorkItemUrl(sourceItem.id) : `#${sourceItem.id}`;
          const targetUrl = this.targetConnector.getWorkItemUrl ? this.targetConnector.getWorkItemUrl(itemResult.target_id) : `#${itemResult.target_id}`;
          this.logger.success(`Created: "${title}" | Source: ${sourceUrl} → Target: ${targetUrl}`);
        }
      } catch (error) {
        this.logger.error('Error syncing new source item', { item_id: sourceItem.id, error: error.message });
        results.errors++;
        results.total++;
      }
    }

    // Step 2: Sync new items from target to source (reverse direction)
    this.logger.info('Step 2: Querying new items from target');
    const targetWorkItems = await this._queryWorkItems(this.targetConnector, workItemIds);
    
    // An item from target is "new" if it doesn't appear in any existing pair
    const newTargetItems = targetWorkItems.filter(item => {
      return !allPairs.some(pair => 
        (pair.source_connector_id === this.config.target_connector_id && pair.source_item_id === item.id.toString()) ||
        (pair.target_connector_id === this.config.target_connector_id && pair.target_item_id === item.id.toString())
      );
    });
    this.logger.info(`Found ${newTargetItems.length} new items in target to sync to source`);
    
    for (const targetItem of newTargetItems) {
      try {
        const itemResult = await this._syncWorkItem(
          targetItem,
          this.targetConnector,
        ...
        // Log the sync action
        if (itemResult.action === 'created') {
          const title = targetItem.fields?.['System.Title'] || targetItem.title || 'Untitled';
          const targetUrl = this.targetConnector.getWorkItemUrl ? this.targetConnector.getWorkItemUrl(targetItem.id) : `#${targetItem.id}`;
          const sourceUrl = this.sourceConnector.getWorkItemUrl ? this.sourceConnector.getWorkItemUrl(itemResult.target_id) : `#${itemResult.target_id}`;
          this.logger.success(`Created: "${title}" | Source: ${sourceUrl} ← Target: ${targetUrl}`);
        }
      } catch (error) {
        this.logger.error('Error syncing new target item', { item_id: targetItem.id, error: error.message });
        results.errors++;
        results.total++;
      }
    }

    // Step 3: Handle existing synced pairs for conflict detection
    this.logger.info('Step 3: Checking existing synced pairs for conflicts');
    const syncedPairs = await db('synced_items')
      .where({ sync_config_id: this.config.id });

    for (const pair of syncedPairs) {
      try {
        // Fetch both work items - handle deleted items
        let sourceItem, targetItem;
        let sourceDeleted = false, targetDeleted = false;
        
        try {
          sourceItem = await this.sourceConnector.getWorkItem(pair.source_item_id);
          // Check if API returned null (item doesn't exist)
          if (!sourceItem) {
            sourceDeleted = true;
            this.logger.info('Source work item deleted (returned null)', { item_id: pair.source_item_id });
          }
        } catch (error) {
          if (error.statusCode === 404 || error.message?.includes('does not exist') || error.message?.includes('not found')) {
            sourceDeleted = true;
            this.logger.info('Source work item deleted', { item_id: pair.source_item_id, error: error.message });
          } else {
        ...
        try {
          targetItem = await this.targetConnector.getWorkItem(pair.target_item_id);
          // Check if API returned null (item doesn't exist)
          if (!targetItem) {
            targetDeleted = true;
            this.logger.info('Target work item deleted (returned null)', { item_id: pair.target_item_id });
          }
        } catch (error) {
          if (error.statusCode === 404 || error.message?.includes('does not exist') || error.message?.includes('not found')) {
            targetDeleted = true;
            this.logger.info('Target work item deleted', { item_id: pair.target_item_id, error: error.message });
          } else {
            throw error; // Re-throw non-404 errors
          }
        }
        
        // If one or both items are deleted, remove the pairing
        if (sourceDeleted || targetDeleted) {
          if (!dryRun) {
            await db('synced_items').where({ id: pair.id }).delete();
            this.logger.info('Removed pairing for deleted work item(s)', {
              synced_item_id: pair.id,
          ...
          results.items.push({
            ...
            action: 'deleted',
            success: true,
            message: sourceDeleted && targetDeleted ? 'Both items deleted' : 
                     sourceDeleted ? 'Source item deleted' : 'Target item deleted'
          });
        ...
        if (sourceCheck.changed && targetCheck.changed) {
          ...
          if (!dryRun) {
            const mapped = await this.mappingEngine.mapWorkItem(sourceItem, this.config.id);
            await this.targetConnector.updateWorkItem(pair.target_item_id, mapped.fields);
            results.updated++;
            
            // Update synced_items record with execution_id
            ...
            // Log the update
            const title = sourceItem.fields?.['System.Title'] || sourceItem.title || 'Untitled';
            const sourceUrl = this.sourceConnector.getWorkItemUrl ? this.sourceConnector.getWorkItemUrl(pair.source_item_id) : `#${pair.source_item_id}`;
            const targetUrl = this.targetConnector.getWorkItemUrl ? this.targetConnector.getWorkItemUrl(pair.target_item_id) : `#${pair.target_item_id}`;
            this.logger.success(`Updated: "${title}" | Source: ${sourceUrl} → Target: ${targetUrl}`);
          }
  ...
  async _syncWorkItem(
    ...
    // Check if already synced - in bidirectional mode, check both directions
    let existing = await db('synced_items')
      .where({
    ...
    // If not found as source, check if this item is the target in a reverse pairing
    let isReversePair = false;
    if (!existing) {
      existing = await db('synced_items')
        .where({
          sync_config_id: this.config.id,
          target_connector_id: fromConnectorId,
          target_item_id: sourceItem.id.toString()
        })
        .first();
    ...
    let targetId = existing ? (isReversePair ? existing.source_item_id : existing.target_item_id) : null;
    let action = 'skipped';
    ...
    if (dryRun) {
      ...
      if (existing) {
        // Update
        if (isReversePair) {
          this.logger.info('Found reverse pair, updating existing relationship', {
            from_item: sourceItem.id,
            to_item: targetId,
            synced_item_id: existing.id
          });
        }
        ...
        try {
          ...
          // If target item was deleted, remove pairing and recreate
          if (updateError.statusCode === 404 || updateError.message?.includes('does not exist') || updateError.message?.includes('not found')) {
            this.logger.info('Target work item deleted, recreating', { 
              old_target_id: targetId,
              source_id: sourceItem.id 
            });
            
            // Remove old pairing
            await db('synced_items').where({ id: existing.id }).delete();
  ...
  async _syncComments(sourceItemId, targetItemId, fromConnector, toConnector, syncedItemId) {
    try {
      ...
      // Get sync options to check if we should include comment metadata
      let syncOptions = {};
      if (this.config.options) {
        try {
          syncOptions = JSON.parse(this.config.options);
        } catch (error) {
          this.logger.warn('Failed to parse sync options for comments', { error: error.message });
        }
      }
      
      const includeCommentMetadata = syncOptions.include_comment_metadata !== false; // Default to true for backward compatibility

      // Sync new comments
      for (const comment of sourceComments) {
        if (!syncedCommentIds.has(comment.id.toString())) {
          try {
            // Format comment text based on configuration
            let commentText = comment.text;
            if (includeCommentMetadata) {
              commentText = `[Synced from source]\n${comment.text}\n\n--- ${comment.createdBy} (${comment.createdDate})`;
            }
  ...
  async _queryWorkItems(connector, workItemIds) {
    if (workItemIds && workItemIds.length > 0) {
      const workItems = await Promise.all(
        workItemIds.map(id => connector.getWorkItem(id).catch(err => {
          console.error(`Failed to fetch work item ${id}:`, err);
          return null;
        }))
      );
      // Filter out null values (work items that don't exist or failed to fetch)
      return workItems.filter(item => item !== null);
    }

    // Check if we should only sync changed items
    let syncOptions = {};
    if (this.config.options) {
      try {
        syncOptions = JSON.parse(this.config.options);
      } catch (error) {
        this.logger.warn('Failed to parse sync options, using defaults', { 
    ...
    if (syncOptions.sync_only_changed && this.config.last_sync_at) {
      lastSyncTime = this.config.last_sync_at;
      this.logger.info('Filtering to only changed items since last sync', { 
    ...
    if (this.config.sync_filter) {
      try {
        ...
        this.logger.warn('Failed to parse sync filter, using default query', { 
          sync_filter: this.config.sync_filter,
          error: error.message 
        });
        query = null;
      }
    ...
    if (!query) {
      ...
      // Add filter for changed items if enabled
      if (lastSyncTime) {
        try {
          const lastSyncDate = new Date(lastSyncTime);
          // Validate it's a valid date
          if (isNaN(lastSyncDate.getTime())) {
            this.logger.warn('Invalid last_sync_at date, skipping change filter', { last_sync_at: lastSyncTime });
          } else {
            const lastSyncDateISO = lastSyncDate.toISOString();
            wiqlWhere += ` AND [System.ChangedDate] > '${lastSyncDateISO}'`;
            this.logger.info('Added change date filter to WIQL', { changed_since: lastSyncDateISO });
          }
        } catch (error) {
          this.logger.warn('Error parsing last_sync_at date, skipping change filter', { 
            last_sync_at: lastSyncTime, 
            error: error.message 
          });
        }
    ...
    try {
      ...
      for (const sourceItem of sourceWorkItems) {
        try {
          ...
          let mappedFields = {};
          try {
            // test: true option prevents side effects in mapping (preview mode)
            const mapped = await this.mappingEngine.mapWorkItem(sourceItem, this.config.id, { test: true });
          ...
          previewItems.push({
            ...
            sourceType: sourceItem.fields?.['System.WorkItemType'],
            title: sourceItem.fields?.['System.Title'],
            state: sourceItem.fields?.['System.State'],
            assignedTo: sourceItem.fields?.['System.AssignedTo']?.displayName,
            action: existingSync ? 'update' : 'create',
            targetId: existingSync ? existingSync.target_item_id : null,
            lastSyncedAt: existingSync ? existingSync.last_synced_at : null,
            syncCount: existingSync ? existingSync.sync_count : 0,
      ...
      return {
        ...
        items: previewItems,
        summary: {
          willCreate: previewItems.filter(i => i.action === 'create').length,
          willUpdate: previewItems.filter(i => i.action === 'update').length,
  ...
  async _reverseMapWorkItem(targetItem, mappings) {
    ...
    // mappings.fields contains the field mappings array
    for (const mapping of mappings.fields) {
      const targetValue = targetItem.fields[mapping.target_reference_name || mapping.target_field_name];
      if (targetValue !== undefined) {
        reverseMapped.fields[mapping.source_reference_name || mapping.source_field_name] = targetValue;
      }
    }
