...
// Path to persist the generated encryption key
const KEY_FILE_PATH = path.join(__dirname, '..', 'data', 'encryption.key');

// Cache the key in memory after first load
let _cachedKey = null;

/**
 * Get encryption key from environment, key file, or generate and persist one
 * Priority: 1) env var  2) key file  3) generate new + save to file
 */
function getEncryptionKey() {
  ...
  // Try reading from persisted key file
  if (!key) {
    try {
      if (fs.existsSync(KEY_FILE_PATH)) {
        key = fs.readFileSync(KEY_FILE_PATH, 'utf8').trim();
        if (key) {
          console.log('Loaded encryption key from', KEY_FILE_PATH);
        }
      }
    } catch (err) {
      console.warn('Could not read key file:', err.message);
    }
  }
  ...
  if (!key) {
    ...
    console.log('Generated new encryption key and saving to', KEY_FILE_PATH);
    try {
      const dir = path.dirname(KEY_FILE_PATH);
      if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
      fs.writeFileSync(KEY_FILE_PATH, key, { mode: 0o600 });
    } catch (err) {
      console.warn('WARNING: Could not save encryption key to file:', err.message);
      console.warn('Set ENCRYPTION_KEY env var for persistent encryption.');
    }
  }

  // Ensure key is proper length
  if (key.length < KEY_LENGTH * 2) { // hex encoding doubles length
    _cachedKey = crypto.scryptSync(key, 'salt', KEY_LENGTH);
    return _cachedKey;
  }

  _cachedKey = Buffer.from(key.substring(0, KEY_LENGTH * 2), 'hex');
  return _cachedKey;
}
