...
class MappingEngine {
  ...
  async loadFieldMappings(syncConfigId) {
    ...
    // Derive mapping_type from database fields and add transformation field
    return mappings.map(mapping => ({
      ...mapping,
      // Derive mapping_type based on database columns
      mapping_type: mapping.is_constant ? 'constant' : 
                    mapping.transformation_function ? 'transformation' : 
                    'direct',
      // Use transformation_function as transformation
      transformation: mapping.transformation_function || null
    }));
  }
  ...
  async loadStatusMappings(syncConfigId) {
    const mappings = await db('sync_status_mappings as sm')
      .select(
        ...
        'ss.status_category as source_category',
        'ts.status_name as target_status_name',
        'ts.status_category as target_category'
      )
  ...
  async mapFields(sourceFields, mappings, context = {}) {
    ...
    console.log(`Mapping ${mappings.fields.length} field mappings`);
    
    // Always ensure System.Title is mapped if it exists in source
    const hasTitleMapping = mappings.fields.some(m => 
      m.target_reference_name === 'System.Title' || m.target_field_name === 'Title'
    );
    
    if (!hasTitleMapping && (sourceFields['System.Title'] || sourceFields['Title'])) {
      const titleValue = sourceFields['System.Title'] || sourceFields['Title'];
      targetFields['System.Title'] = titleValue;
      console.log('Auto-mapped required field: Title -> System.Title (required field)');
    }
    ...
    for (const mapping of mappings.fields) {
      try {
        ...
        switch (mapping.mapping_type) {
          ...
          default:
            console.warn(`Unknown mapping type: ${mapping.mapping_type} for field: ${mapping.source_field_name || 'unknown'}`);
            continue;
        }
        ...
        if (value != null) {
          ...
          console.log(`Mapped field: ${mapping.source_field_name} -> ${targetKey} (type: ${mapping.mapping_type})`);
        }
      } catch (error) {
    ...
    console.log(`Mapped ${Object.keys(targetFields).length} target fields`);
    return targetFields;
  }
  ...
  async mapWorkItem(sourceWorkItem, syncConfigId, context = {}) {
    ...
    // Map type if present - check both .type and fields['System.WorkItemType']
    const sourceType = sourceWorkItem.type || sourceWorkItem.fields?.['System.WorkItemType'];
    if (sourceType) {
      mapped.type = this.mapType(sourceType, mappings);
    }
...
// Export class (not singleton - each sync needs its own instance)
